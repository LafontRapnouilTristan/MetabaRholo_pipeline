...
EcophyCofog::Library(c("reshape2","ggplot2","stringr","cowplot",
                       ...
                       "speedyseq","metabaR"))
load("data/raw_metablists_art2.RData")
set.seed(973)
```
...
```{r}
ggsave("figures/reads_and_motus_art2.jpg",diagplot, height = 4, width = 6 )
```
...
```{r}
ggsave("figures/motu_read_cor_art2.jpg",moturead, height = 4, width = 6 )
```
...
# ggsave("figures/raref_art2.jpg",raref, height = 10, width = 15 )
```
...
```{r}
ggsave("figures/conta_prop_art2.jpg",contaprop, height = 4, width = 6 )
```
...
```{r}
ggsave("figures/tax_boots_art2.jpg",plottaxboots, height = 4, width = 6 )
```
...
```{r}
ggsave("figures/seq_depth_art2.jpg",plotseqdepth, height = 4, width = 6 )
```
...
ggsave("figures/tag_jump_art2.jpg",plottagjump, height = 8, width = 13 )
```
...
ggsave("figures/noise_motu_art2.jpg",plotnoisemotu, height = 5, width = 8 )
```
...
ggsave("figures/noise_pcr_art2.jpg",plotnoisepcr, height = 5, width = 8)
```
...
ggsave("figures/sum_pipeline_art2.jpg",sumpipelineplots, height = 3, width = 6)
```
...
ggsave("figures/motu_read_cor_post_art2.jpg",moturead, height = 4, width = 6 )
```
...
ggsave("figures/tax_boots_post_art2.jpg",plottaxbootspost, height = 4, width = 6 )
```
...
# }
# raref <- ggarrange(plotlist =  raref_plots, ncol = 1, common.legend = T, legend ="bottom")
# ggsave("figures/raref_post_vnart2.jpg",raref, height = 20, width = 15 )
```
...
sammat <- left_join(arrange(tmp$samples,sample_id),arrange(tmp$pcrs,sample_id))
...
ggsave("figures/betadiv_art2.jpg",betadiv, height = 12, width = 40 )
```
...
ggsave("figures/betadiv2_art2.jpg",betadiv2, height = 12, width = 40 )
```
...
save.image("data/cl_metablists_physeq_art2.RData")
```
