...
for (i in 1:length(list_reads)){
  ...
  list_filters[[i]] <- dplyr::filter(list_filters[[i]], !ID %in% c(setdiff(sort(list_filters[[i]]$ID),sort(list_samples[[i]]$sample_id))))
...
```{r}
list_samples[[1]] %<>% mutate(organ= ifelse(grepl("_F",list_samples[[1]]$sample_id)==T,"LEAVES",
                                                               ifelse(grepl("_R",list_samples[[1]]$sample_id)==T,"ROOTS",
                                                                      ifelse(grepl("_GR",list_samples[[1]]$sample_id)==T,"SEEDS","SOIL")))) %>%
  mutate(project = ifelse(organ =="SEEDS","HOLOBROM_SEEDS","HOLOBROM_MERES"))

list_samples[[2]] %<>% 
  mutate(organ = ifelse( grepl("(_Rp)|([0-9]{3}R)",list_samples[[2]]$sample_id)==T,"ROOTS",
                                                         ...
                                                         ifelse( grepl("GR",list_samples[[2]]$sample_id)==T,"SEEDS",
                                                                 ifelse( grepl("ED",list_samples[[2]]$sample_id)==T,"ENDOPHYTES",
                                                                         ifelse( grepl("EP",list_samples[[2]]$sample_id)==T,"EPIPHYTES",
                                                                                 ifelse( grepl("[0-9]{3}(S|C)",list_samples[[2]]$sample_id)==T,"RHIZOSPHERE",
                                                                                         ifelse( grepl("CT|EXT|PCR|TPOS",list_samples[[2]]$sample_id)==T,"CONTROL","SOIL")
                                                                                 )))))))))) %>%
  mutate(project = ifelse(organ %in% c("ENDOPHYTES","EPIPHYTES","RHIZOSPHERE") | grepl("[0-9]{3}R",list_samples[[2]]$sample_id)==T,"INT_M",list_pcrs[[2]]$project))

list_samples[[3]] %<>% 
  mutate(organ = ifelse( grepl("_R|(-R)|([0-9]{3}R)",list_samples[[3]]$sample_id)==T,"ROOTS",
                         ifelse( grepl("FL",list_samples[[3]]$sample_id)==T,"FLOWER",
                                 ifelse( grepl("FE",list_samples[[3]]$sample_id)==T,"LEAVES",
                                         ifelse( grepl("FR",list_samples[[3]]$sample_id)==T,"FRUIT",
                                                 ifelse( grepl("RJ",list_samples[[3]]$sample_id)==T,"RESHOOT",
                                                         ifelse( grepl("GR",list_samples[[3]]$sample_id)==T,"SEEDS",
                                                                 ifelse( grepl("ED",list_samples[[3]]$sample_id)==T,"ENDOPHYTES",
                                                                         ifelse( grepl("EP",list_samples[[3]]$sample_id)==T,"EPIPHYTES",
                                                                                 ifelse( grepl("[0-9]{3}(S|C)",list_samples[[3]]$sample_id)==T,"RHIZOSPHERE",
                                                                                         ifelse( grepl("CT|EXT|PCR|TPOS",list_samples[[3]]$sample_id)==T,"CONTROL","SOIL")
                                                                                 )))))))))) %>%
  mutate(project = ifelse(organ %in% c("ENDOPHYTES","EPIPHYTES","RHIZOSPHERE") | grepl("[0-9]{3}R",list_samples[[3]]$sample_id)==T,"INT_M",list_pcrs[[3]]$project))

list_samples[[4]] %<>% 
  mutate(organ = ifelse( grepl("_R|(-R)|([0-9]{3}R)",list_samples[[4]]$sample_id)==T,"ROOTS",
                         ifelse( grepl("FL",list_samples[[4]]$sample_id)==T,"FLOWER",
                                 ifelse( grepl("FE",list_samples[[4]]$sample_id)==T,"LEAVES",
                                         ifelse( grepl("FR",list_samples[[4]]$sample_id)==T,"FRUIT",
                                                 ifelse( grepl("RJ",list_samples[[4]]$sample_id)==T,"RESHOOT",
                                                         ifelse( grepl("GR",list_samples[[4]]$sample_id)==T,"SEEDS",
                                                                 ifelse( grepl("ED",list_samples[[4]]$sample_id)==T,"ENDOPHYTES",
                                                                         ifelse( grepl("EP",list_samples[[4]]$sample_id)==T,"EPIPHYTES",
                                                                                 ifelse( grepl("[0-9]{3}(S|C)",list_samples[[4]]$sample_id)==T,"RHIZOSPHERE",
                                                                                         ifelse( grepl("CT|EXT|PCR|TPOS",list_samples[[4]]$sample_id)==T,"CONTROL","SOIL")
                                                                                 )))))))))) %>%
  mutate(project = ifelse(organ %in% c("ENDOPHYTES","EPIPHYTES","RHIZOSPHERE") | grepl("[0-9]{3}R",list_samples[[4]]$sample_id)==T,"INT_M",list_pcrs[[4]]$project))

list_samples[[5]] %<>% 
  mutate(organ= ifelse(grepl("_F",list_samples[[5]]$sample_id)==T,"LEAVES",
                       ifelse(grepl("_R",list_samples[[5]]$sample_id)==T,"ROOTS","CONTROL")),
         project= "HOLOBROM_PLANTULES")


list_samples[[6]] %<>%
  mutate(organ= ifelse(grepl("_F",list_samples[[6]]$sample_id)==T,"LEAVES",
                       ifelse(grepl("_R",list_samples[[6]]$sample_id)==T,"ROOTS","CONTROL")),
         project= "HOLOBROM_PLANTULES")
...
save.image("data/raw_metablists.RData")
```
...
for (i in 1:length(folder_names)){
  ...
  for(j in 1:length(colnames_tmp)){
    ...
    colnames(conf) <- paste0("conf_",colnames_tmp[j])
...
```{r}
# SET THRESHOLD: test 0.05

plot_noise_motu <- NULL
motu_noise_tab <- NULL
for (i in 1:length(folder_names)){
  
  data_temp <- metablists[[folder_names[i]]]["t_0.05"][[1]]
...
data_temp <- metablists[[folder_names[i]]][["t_0"]]
...
for (i in 1:length(physeqlists)){ 
  ...
  mds1 <- plot_ordination(physeq,ordinate(physeq,
                                          method = "NMDS",
                                          distance = "bray"),
                          type = "taxa",
                          color = "conf_Phylum")
  mds2 <- plot_ordination(physeqcl,ordinate(physeqcl,
                                   method = "NMDS",
                                   distance = "bray"),
                          type = "taxa",
                          color = "conf_Phylum")
  ...
  a <- mds1 + labs(color = "Phylum") +
    ...
    ggtitle(paste0("Raw data",folder_names[[i]]))
  b <- mds2 + labs(color = "Phylum") +
    ggtitle(paste0("Clean data",folder_names[[i]]))
