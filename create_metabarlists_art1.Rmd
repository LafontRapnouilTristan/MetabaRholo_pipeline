...
prepare data_pcrs
```{r}
filtid <- list_filters$mother_16s2$X2 # ID 16s
sampid <- list_samples[[1]]$sample_id # ID samples

list_filters$mother_16s2$X2[which(filtid %in% sampid==F)] <- gsub("-","_",list_filters$mother_16s2$X2[which(filtid %in% sampid==F)]) # replace - by _
c(setdiff(sort(list_filters[[i]]$X2),sort(list_samples[[i]]$sample_id))) # check if ngs and sample names match
