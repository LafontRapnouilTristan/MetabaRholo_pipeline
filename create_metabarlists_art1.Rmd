...
prepare data_pcrs
```{r}
filtid <- list_filters$mother_16s2$X2
sampid <- list_samples[[1]]$sample_id

list_filters$mother_16s2$X2[which(filtid %in% sampid==F)] <- gsub("-","_",list_filters$mother_16s2$X2[which(filtid %in% sampid==F)])
c(setdiff(sort(list_filters[[i]]$X2),sort(list_samples[[i]]$sample_id)))
c(setdiff(sort(list_samples[[i]]$sample_id),sort(list_filters[[i]]$X2)))
...
list_samples[[1]] %<>%
  mutate(organ = ifelse( grepl("_R|(-R)|([0-9]{1}R)",list_samples[[1]]$sample_id)==T,"ROOTS",
                                                         ...
                                                         ifelse( grepl("GR",list_samples[[1]]$sample_id)==T,"SEEDS",
                                                                 ifelse( grepl("ED",list_samples[[1]]$sample_id)==T,"ENDOPHYTES",
                                                                         ifelse( grepl("EP",list_samples[[1]]$sample_id)==T,"EPIPHYTES",
                                                                                 ifelse( grepl("[0-9]{1}(S|C)",list_samples[[1]]$sample_id)==T,"RHIZOSPHERE",
                                                                                         ifelse( grepl("CT|EXT|PCR|TPOS",list_samples[[1]]$sample_id)==T,"CONTROL","SOIL")
                                                                                 )))))))))) %>%
  mutate(project = ifelse(organ %in% c("ENDOPHYTES","EPIPHYTES","RHIZOSPHERE") | grepl("[0-9]{1}R",list_samples[[1]]$sample_id)==T,"INT_M",list_pcrs[[1]]$project))
                                                         ...
                                                         ifelse( grepl("GR",list_samples[[2]]$sample_id)==T,"SEEDS",
                                                                 ifelse( grepl("ED",list_samples[[2]]$sample_id)==T,"ENDOPHYTES",
                                                                         ifelse( grepl("EP",list_samples[[2]]$sample_id)==T,"EPIPHYTES",
                                                                                 ifelse( grepl("[0-9]{2}(S|C)",list_samples[[2]]$sample_id)==T,"RHIZOSPHERE",
...
save.image("data/raw_metablists_art1.RData")
```