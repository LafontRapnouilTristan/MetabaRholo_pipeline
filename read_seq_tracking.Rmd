...
for (i in 1:length(list_files)){
  ...
  tmp <-  read.csv(paste0("OBIholo_test/results/",current_name,"/",current_name,"_seq_tracking.csv"),sep = "\t")
  ...
  tmp_derep <- read_delim(paste0("OBIholo_test/results/",current_name,"/",current_name,"_derep.sampstat"), " ")
  tmp_clean <- read_delim(paste0("OBIholo_test/results/",current_name,"/",current_name,"_clean.sampstat2"), " ")
  tmp_clust <- read_delim(paste0("OBIholo_test/results/",current_name,"/",current_name,"_clust.sampstat2"), " ")
  tmp_agg <- read_delim(paste0("OBIholo_test/results/",current_name,"/",current_name,"_agg.sampstat2"), " ")
  
  # Join all data frames in list
  list_df = list(tmp_derep,tmp_clean,tmp_clust,tmp_agg)
  tmp_merged <- Reduce(function(x, y) merge(x, y, all=T), list_df)
  
  tmp_reads_dml <- read_delim(paste0("OBIholo_test/results/",current_name,"/",current_name,"_reads_tracking_deml.txt"), " ", col_names = F)[,1:2] 
  tmp_seq_dml <- read_delim(paste0("OBIholo_test/results/",current_name,"/",current_name,"_seq_tracking_deml.txt"), "\t", col_names = F)[,1:2]
  tmp_reads_filt <- read_delim(paste0("OBIholo_test/results/",current_name,"/",current_name,"_reads_tracking_basfilt.txt"), " ", col_names = F)[,1:2]
  tmp_seq_filt <- read_delim(paste0("OBIholo_test/results/",current_name,"/",current_name,"_seq_tracking_basfilt.txt"), "\t", col_names = F)[,1:2]
  ...
  ngs <- arrange(read_delim(paste0("OBIholo_test/resources/",current_name,"/",current_name,"_ngsfilter.tab"),"\t", escape_double = F, trim_ws = T, col_names = F),X2)
  ...
  melted_tmp_sample <- melt(tmp_merged, id.vars =  c('sample','type','proj'),  measure.vars = 2:13 ) %>% mutate(steps = str_extract(variable,".+(?=\\.)"),
    ...
    mutate(steps = fct_relevel(steps,"dml","filt","derepl","clean","clust","agg"),
