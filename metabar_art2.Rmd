...
folder_names <- folder_names[c(8:9)]
...
prepare data_pcrs
```{r}
filtid <- list_filters$seed_seedling_merged16s$X2 # ID 16s
sampid <- list_samples[[1]]$sample_id # ID samples

list_filters$seed_seedling_merged16s$X2[which(filtid %in% sampid==F)] <- gsub("-","_",list_filters$seed_seedling_merged16s$X2[which(filtid %in% sampid==F)]) # replace - by _
...
list_filters$seed_seedling_merged16s %<>% filter(!X2%in%c(setdiff(sort(list_filters[[1]]$X2),sort(list_samples[[1]]$sample_id))))
c(setdiff(sort(list_filters[[1]]$X2),sort(list_samples[[1]]$sample_id)))



filtid <- list_filters$seed_seedling_mergedITS2$X2 # ID ITS2
sampid <- list_samples[[2]]$sample_id # ID samples

list_filters$seed_seedling_mergedITS2$X2[which(filtid %in% sampid==F)] <- gsub("-","_",list_filters$seed_seedling_mergedITS2$X2[which(filtid %in% sampid==F)]) # replace - by _
...
list_filters$seed_seedling_mergedITS2 %<>% filter(!X2%in%c(setdiff(sort(list_filters[[2]]$X2),sort(list_samples[[2]]$sample_id))))
c(setdiff(sort(list_filters[[2]]$X2),sort(list_samples[[2]]$sample_id)))
# got some duplication cuz no setdiff but different length

list_filters[[2]] <- list_filters[[2]][!duplicated(list_filters[[2]]$X2), ]
```
...
list_samples[[1]] %<>%
  mutate(organ = ifelse( grepl("_R$",list_samples[[1]]$sample_id)==T,"ROOTS",
                         ifelse( grepl("_F$",list_samples[[1]]$sample_id)==T,"LEAVES",
                                 ifelse( grepl("_GR$",list_samples[[1]]$sample_id)==T,"SEEDS","CONTROL")))) %>%
  mutate(project = "HOLOBROM_PLANTULES")

list_samples[[2]] %<>%
  mutate(organ = ifelse( grepl("_R$",list_samples[[2]]$sample_id)==T,"ROOTS",
                         ifelse( grepl("_F$",list_samples[[2]]$sample_id)==T,"LEAVES",
                                 ifelse( grepl("_GR$",list_samples[[2]]$sample_id)==T,"SEEDS","CONTROL")))) %>%
  mutate(project = "HOLOBROM_PLANTULES")
```
...
save.image("data/raw_metablists_art2.RData")
```