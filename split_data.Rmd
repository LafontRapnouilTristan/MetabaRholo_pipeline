...
```{r}
sub_mother_16s
meco_mo16s <- file2meco::phyloseq2meco(sub_mother_16s)

meco_mo16s$sample_sums()
```
...
```{r}
plot_richness(sub_seedling_16s)
```


```{r}
plot_bar(sub_seedling_16s)
```


```{r}
ntaxa(sub_seedling_16s)
nsamples(sub_seedling_16s)

reads_samples <- data.frame(reads=rowSums(otu_table(sub_seedling_16s)),
                            samples=rownames(otu_table(sub_seedling_16s)),
                            richness=rowSums(otu_table(sub_seedling_16s) != 0))
