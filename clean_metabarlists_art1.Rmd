...
EcophyCofog::Library(c("reshape2","ggplot2","stringr","cowplot",
                       ...
                       "speedyseq","metabaR"))
load("data/raw_metablists_art1.RData")
set.seed(973)
```
...
folder_names <- folder_names[c(-3,-4)]
...
```{r}
ggsave("figures/reads_and_motus_art1.jpg",diagplot, height = 4, width = 6 )
```
...
```{r}
ggsave("figures/motu_read_cor_art1.jpg",moturead, height = 8, width = 6 )
```
...
# ggsave("figures/raref_art1.jpg",raref, height = 20, width = 15 )
```
...
```{r}
ggsave("figures/conta_prop_art1.jpg",contaprop, height = 8, width = 6 )
```
...
for (i in 1:length(folder_names)){
  ...
  target_kingdom <- ifelse(target=="16s2","Bacteria|Archaea","Fungi") # what do we target
...
```{r}
ggsave("figures/tax_boots_art1.jpg",plottaxboots, height = 8, width = 6 )
```
...
```{r}
ggsave("figures/seq_depth_art1.jpg",plotseqdepth, height = 8, width = 6 )
```
...
ggsave("figures/tag_jump_art1.jpg",plottagjump, height = 12, width = 13 )
```
...
ggsave("figures/noise_motu_art1.jpg",plotnoisemotu, height = 9, width = 8 )
```
...
ggsave("figures/noise_pcr_art1.jpg",plotnoisepcr, height = 8, width = 6)
```
...
ggsave("figures/sum_pipeline_art1.jpg",sumpipelineplots, height = 6, width = 6)
```
...
ggsave("figures/motu_read_cor_post_art1.jpg",moturead, height = 8, width = 6 )
```
...
ggsave("figures/tax_boots_post_art1.jpg",plottaxbootspost, height = 8, width = 6 )
```
...
raref <- ggarrange(plotlist =  raref_plots, ncol = 1, common.legend = T, legend ="bottom")
ggsave("figures/raref_post_vnart1.jpg",raref, height = 20, width = 15 )
```
...
ggsave("figures/betadiv_art1.jpg",betadiv, height = 12, width = 30 )
```
...
ggsave("figures/betadiv2_art1.jpg",betadiv2, height = 12, width = 30 )
```
...
save.image("data/cl_metablists_physeq_art1.RData")
```
